		...
		estraverse.traverse(ast, {
			enter: function (node, parent) {
				if ((node.type === "FunctionDeclaration" && node.id.name !== "__profiled__") || node.type === "FunctionExpression") {
					this.functionID++;
					if (node.id)
						idToFunctionName[this.functionID] = node.id.name;
					else if (node.type === "FunctionExpression" && parent && parent.type === "VariableDeclaration")
						idToFunctionName[this.functionID] = parent.id.name;
					else if (node.type === "FunctionExpression" && parent && parent.type === "Property")
						idToFunctionName[this.functionID] = parent.key.name;
					else {
						idToFunctionName[this.functionID] = '(anonymous: ' + this.functionID.toString() + ')';
					}
					idToURL[this.functionID] = url ? url : '';
					idToLocation[this.functionID] = (node.loc.start.line  ? node.loc.start.line : 1) + ',' + (node.loc.start.column ? node.loc.start.column : 1);
					...
					function_id_node.raw = this.functionID.toString();
					function_id_node.value = this.functionID;
